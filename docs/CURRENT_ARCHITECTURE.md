...
Project Reika has four workspace layers: Electron/React client, device-local Reika Node, relay server, and shared protocol. The canonical protocol is `shared/protocol/index.ts`; run `npm run sync:protocol` after changes and `npm run check:protocol` before shipping.
...
- Windows desktop packaging with the Reika Node embedded.
